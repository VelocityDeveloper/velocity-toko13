...
function vd_search()
{
    ob_start(); ?>
    <div class="vsearch">
        <form method="get" id="searchform" action="<?php echo esc_url(home_url('/')); ?>" style="max-width:350px;">
            <label class="sr-only" for="s"><?php esc_html_e('Search', 'justg'); ?></label>
            <div class="input-group border rounded">
                <input class="field form-control rounded-start border-0" id="s" name="s" type="text" placeholder="Cari Produk" value="<?php the_search_query(); ?>" required>
                <span class="input-group-append bg-white rounded-end">
                    <button type="submit" class="submit btn btn-sm h-100 py-2 px-3 border-start">
